...
- **Not VPN/`utun`-related.** Earlier failure captures showed kernel defaults surviving only via `utun0-7` (that machine ran Tailscale, `fd7a:115c:a1e0::/48`). The vanilla machine has no VPN and still loses the route — the `utun` defaults were incidental, not causal.
...
1. **Apple Thread Border Routers** (Apple TV, HomePod mini) — in the May 18 2026 capture these send RAs with **router lifetime `0`** plus **Route Information Options (RIO)** for ULA prefixes (e.g. `fd3a:3b8:3133::/64`, `fd9e:350e:ee32::/64`). With lifetime 0 they should *not* enter the Default Router List at all.
...
1. **Event-driven instead of polling.** Subscribe to a **`PF_ROUTE` socket** and react to `RTM_DELROUTE` for the default route, instead of polling SystemConfiguration + the routing table. Catches the deletion the instant it happens.
...
- **Best self-buildable progress:** prevent the trigger (`pf` whitelist / network RA Guard) and keep collecting `RA@loss` evidence; optionally move the monitor to an event-driven `PF_ROUTE` design.

---
*Living analysis note. Updated 2026-06-26 with: factory-new-hardware reproduction (VPN/interface/config ruled out), the live multi-default-router `ndp -rn` observation, and the automatic `RA@loss` evidence capture.*
